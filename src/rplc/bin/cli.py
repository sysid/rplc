...
def workmux(
    ...
    """
    Generate .workmux.yaml configuration for workmux integration.

    Creates a workmux configuration file with:
    - post_create hooks for direnv setup
    - files.symlink entries for all rplc-managed files

    Only works when configuration is already swapped-in.
    """
    # Use environment variables as defaults if options not provided
    proj_dir = proj_dir or get_default_proj_dir()
    mirror_dir = mirror_dir or get_default_mirror_dir()
    ...
    # Check if at least one file is swapped-in
    swapped_count = sum(
        1 for cfg in manager.configs if manager._get_sentinel_path(cfg).exists()
    )
    if swapped_count == 0:
        console.print("[red]Error: Configuration must be swapped-in first[/red]")
        console.print("[dim]Run 'rplc swapin' before generating workmux config.[/dim]")
        raise typer.Exit(1)

    # Gather all managed files from configs
    symlink_entries = []
    for cfg in manager.configs:
        try:
            rel_path = cfg.source_path.relative_to(proj_dir.resolve())
        except ValueError:
            rel_path = cfg.source_path
        # Add trailing slash for directories
        entry = str(rel_path) + ("/" if cfg.is_directory else "")
        symlink_entries.append(entry)

    # Include .workmux.yaml itself (will be added to config after YAML is written)
    if ".workmux.yaml" not in symlink_entries:
        symlink_entries.append(".workmux.yaml")

    # Include .venv if it exists (useful for Python projects)
    venv_path = proj_dir.resolve() / ".venv"
    if venv_path.is_dir() and ".venv/" not in symlink_entries and ".venv" not in symlink_entries:
        symlink_entries.append(".venv/")

    # Check .envrc: symlink → post_create hook, real file → symlink entry
    envrc_path = proj_dir.resolve() / ".envrc"
    envrc_is_symlink = envrc_path.is_symlink()
    if envrc_path.exists() and not envrc_is_symlink:
        if ".envrc" not in symlink_entries:
            symlink_entries.append(".envrc")
    ...
    if envrc_is_symlink:
        yaml_lines.append("  - ln -s $SOPS_PATH/dot.envrc .envrc")
    yaml_lines.extend([
        "  - direnv allow",
        "",
        "files:",
    ...
    # Write .workmux.yaml
    workmux_path = proj_dir.resolve() / ".workmux.yaml"
    workmux_path.write_text(yaml_content)
    console.print(f"[green]Created {workmux_path}[/green]")

    # Add .workmux.yaml to rplc config if not present
    added = ConfigParser.add_config_entry(config_file, ".workmux.yaml")
    if added:
        console.print(f"[green]Added '.workmux.yaml' to {config_file}[/green]")
    else:
        console.print(f"[dim]'.workmux.yaml' already in {config_file}[/dim]")
